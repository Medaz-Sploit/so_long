...
void	render(t_so_long *so_long)
{
	...
	so_long->win_ptr = mlx_new_window(so_long->mlx_ptr, so_long->map.WIDTH * TILE_SIZE, so_long->map.HEIGHT * TILE_SIZE, "SO_LONG");
	if (so_long->img == 0)
...
static void	is_collectible(t_so_long *so_long)
{
	...
	while (k < so_long->game->count_coll)
	{
		if (so_long->game->coll[k].X == so_long->game->player.X
			&& so_long->game->coll[k].Y == so_long->game->player.Y)
		{
...
int        game_update(t_so_long *so_long)
{
	...
	if (so_long->game->exit.X == so_long->game->player.X
		&& so_long->game->exit.Y == so_long->game->player.Y)
		if (so_long->game->count_coll == so_long->game->player_coll)
